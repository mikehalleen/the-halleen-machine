...
# Default project-files workspace. Only auto-created when the user keeps this
# exact value — a custom path means they're managing that directory themselves.
WORKSPACE_DEFAULT = "./projects"
...
def detect_agent_python() -> str:
    """Relative path to repo venv python for [agent].python (empty if not found)."""
    if IS_WINDOWS:
        rel = "./the-machine-ui-venv/Scripts/python.exe"
        candidate = Path("the-machine-ui-venv") / "Scripts" / "python.exe"
    else:
        rel = "./the-machine-ui-venv/bin/python"
        candidate = Path("the-machine-ui-venv") / "bin" / "python"
...
def generate_config_toml(
    ...
    agent_python = norm(agent_python or detect_agent_python())
...
[agent]
# Python for thm-agent CLI and pipeline scripts (auto-detected at setup)
python = "{agent_python}"
...
def run_interactive():
    ...
    print("Use './projects' to keep them with the app, or an absolute path.")
    print("('samples/' is a separate folder for curated example looks/workflows.)")
    print()

    current_workspace = current_values.get("workspace", WORKSPACE_DEFAULT)
    workspace = prompt_path("Workspace directory", default=current_workspace)
    if workspace == WORKSPACE_DEFAULT:
        Path(workspace).mkdir(parents=True, exist_ok=True)
        print(f"  Created {Path(workspace).resolve()}")
    ...
    config_content = generate_config_toml(
        ...
        api_base=api_base,
        agent_python=detect_agent_python(),
    )
    ...
    if prompt_yes_no("Install ComfyUI custom nodes?"):
        ...
        try:
            ...
            print()
            if failed > 0:
                print(f"{failed} node(s) failed to install. See output above.")
    ...
    print()
    skill_src = Path("thm-agent") / "skill" / "thm-agent-client"
    if prompt_yes_no("Install the THM agent skill into a directory now?", default=False):
        dest = input("Target skills directory (Enter to skip): ").strip()
        if dest:
            try:
                from install_skill import install_skill as _install_skill
                target = _install_skill(Path(dest), force=False)
                print(f"Installed skill to: {target}")
            except Exception as e:
                print(f"Skill install skipped: {e}")
                print("   Install later with: python install_skill.py --dest <dir>")
        else:
            print("Skipped.")
    print(f"Agent skill (generic, client-neutral) lives at: {skill_src}")
    print(f"   To use it in your agent client, see {skill_src / 'INSTALL.md'}")
...
def run_defaults():
    ...
    Path(WORKSPACE_DEFAULT).mkdir(parents=True, exist_ok=True)
    print(f"  Created {Path(WORKSPACE_DEFAULT).resolve()}")
    ...
    if unconfigured or missing_video:
        ...
        if unconfigured:
            print("Some config models were not found and left unconfigured:")
            for k in unconfigured:
                print(f"   - {k}")
        
        if missing_video:
            print()
            print("Some video generation models are missing:")
